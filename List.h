...
class List{
    ...
    public:
        List();
        virtual ~List();
        virtual Node<T>* insert (T data, int pos) = 0; //empty as the derived class will do the inserting into inked lists (check polymorphism)
        virtual int remove (T data)=0;//derive will implement and remove data from the list/ return the index of tthe node removed  
